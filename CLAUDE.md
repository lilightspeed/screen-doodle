...
| **撤销/重做** | Ctrl+Z / Ctrl+Y，笔迹历史 + 选区移动三阶段撤销 |
| **套索选择/移动** | 自由绘制套索选区，选中笔迹可拖拽移动；跨越边界的笔迹自动切分为内/外两部分 |
...
SELECT 工具通过自由绘制套索（多段线）定义多边形选区：

```
鼠标拖拽 → 收集 QPointF 列表 → _close_lasso()
     ↓
多边形创建 → _split_stroke_at_polygon() 分割跨越边界的笔迹
     ↓
Stroke 列表重建：原始笔迹 → 外部段(保留在原位) + 内部段(标记选中)
     ↓
_selected_stroke_indices = {内部段索引}
     ↓
鼠标拖拽 → _translate_selection(delta) 平移选中段的所有点
     ↓
_commit_move() 保存逆位移到 _move_undo_stack，支持 Ctrl+Z 撤销
```

### 三段式撤销状态机

选区操作的 `Ctrl+Z` 按优先级分三阶段执行：

```
1. _move_undo_stack 非空 → 撤销最后一次选区拖动（反向位移）
2. _pre_split_strokes 非空 → 撤销套索分割（恢复原始笔迹，保留分割后新画的笔迹）
3. stroke_manager.undo() → 撤销笔迹绘制操作
```

- **`_move_undo_stack`**：存储 `{indices, delta}` 对，每次拖动提交后入栈。当 `_close_lasso` 发生分割时清空（因为分割重建了笔画列表，旧索引失效）。
- **`_pre_split_strokes`**：套索闭合时的笔画列表快照。恢复时通过 `_post_split_count` 记录分割后的笔画数，只替换分割产生的笔画，保留用户后续画的笔画。
- 分割 → 拖动多次 → Ctrl+Z × N 依次撤销后几次拖动 → Ctrl+Z 撤销分割 → Ctrl+Z 撤销笔迹
...
- [x] 套索选择 + 选区拖动（含边界笔画自动切分）
- [x] 三段式撤销（移动撤销 → 分割撤销 → 笔迹撤销）
- [x] 清除全部
